...
type Produkt = { name: string; desc: string; link: string; internerLink?: string; internerLinkLabel?: string };
  ...
  },
  {
    ...
    produkte: [
      ...
      },
      {
        ...
        desc: 'Pflicht! Eigenes Sieb für glutenfreie Nudeln – kein Teilen mit normalen Nudeln, sonst Kreuzverunreinigung. Tipp: GF-Nudeln immer zuerst abschütten, dann erst die normalen – so ist das Sieb sauber wenn es drauf ankommt.',
...
function ProduktKarte({ name, desc, link, internerLink, internerLinkLabel }: {
  name: string; desc: string; link: string;
  internerLink?: string; internerLinkLabel?: string;
}) {
    ...
    <div style={{
      ...
      {open && (
        <div style={{ padding: '0.75rem 1rem 1rem', borderTop: '1px solid var(--border)' }}>
          <p style={{ fontSize: '0.825rem', color: 'var(--text-mid)', lineHeight: 1.7, margin: '0 0 0.875rem' }}>
            {desc}
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'center' }}>
            ...
            {internerLink && (
              <Link href={internerLink} style={{ fontSize: '0.8rem', color: 'var(--text-light)', textDecoration: 'none' }}>
                {internerLinkLabel} →
              </Link>
            )}
          </div>
    ...
    <section className="section">
      <div className="container" style={{ maxWidth: '820px' }}>
        ...
        <div style={{
          ...
          Alle Empfehlungen basieren auf eigener Erfahrung. Gleichartige Produkte anderer Hersteller sind natürlich genauso geeignet.
