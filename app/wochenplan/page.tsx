...
type Recipe = {
  ...
  vegetarisch: boolean;
  vegan: boolean;
  laktosefrei: boolean;   // komplett ohne Laktose / substituierbar
  fruktosearm: boolean;   // wenig Fruktose
  nussfrei: boolean;      // keine Nüsse / Mandelmehl
  eifrei: boolean;        // ohne Ei
  einfrierbar: boolean;
};
...
const ALL_RECIPES: Recipe[] = [
  {
    ...
    vegetarisch: true, vegan: false,
    laktosefrei: true,  // mit Pflanzenmilch
    fruktosearm: false, // Apfel enthält Fruktose
    nussfrei: true, eifrei: true,
    einfrierbar: false,
  },
  {
    title: 'Pfannkuchen',
    ...
    vegetarisch: true, vegan: false,
    laktosefrei: true, fruktosearm: true,
    nussfrei: true, eifrei: false,
    einfrierbar: true,
  },
  {
    ...
    minuten: 25,
    naturalGf: false, kleinkind: true,
    vegetarisch: true, vegan: true,
    laktosefrei: true, fruktosearm: false,
    nussfrei: true, eifrei: true,
    einfrierbar: true,
  },
  {
    ...
    vegetarisch: true, vegan: false,
    laktosefrei: false, fruktosearm: true,
    nussfrei: true, eifrei: true,
    einfrierbar: true,
  },
  {
  ...
  },
  {
    ...
    vegetarisch: true, vegan: true,
    laktosefrei: true, fruktosearm: true,
    nussfrei: false, // Erdnussbutter (ersetzbar)
    eifrei: true,
    einfrierbar: true,
  },
  {
    ...
    minuten: 35,
    naturalGf: true, kleinkind: true,
    vegetarisch: true, vegan: true, // mit Kokosmilch pur
    laktosefrei: true, fruktosearm: true,
    nussfrei: true, eifrei: true,
    einfrierbar: false,
  },
...
export default function WochenplanPage() {
  ...
  // Aktive Filter-Anzahl (ohne woche)
    ...
    <>
      ...
      <section style={{ background: 'var(--green-deep)', padding: '2.75rem 0 2.25rem' }}>
        <div className="container">
          ...
          <p style={{ color: 'rgba(255,255,255,0.75)', maxWidth: '500px' }}>
            Stell deine Präferenzen ein – wir stellen deinen Plan aus unseren Rezepten zusammen.
          </p>
        </div>
      </section>
      ...
      <section style={{ background: 'var(--cream-dark)', borderBottom: '1px solid var(--border)', padding: '1.75rem 0' }}>
        <div className="container" style={{ maxWidth: '860px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.5rem 2rem' }}>
            ...
            <FilterGroup label="🥦 Ernährungsweise">
              ...
              <Pill label="🌱 Vegan"       active={prefs.vegan}       onToggle={() => {
                setPrefs(p => {
                  const newVegan = !p.vegan;
                  // Vegan impliziert vegetarisch
                  return { ...p, vegan: newVegan, vegetarisch: newVegan ? true : p.vegetarisch };
                });
              }} />
            </FilterGroup>
          ...
          {/* Woche + Aktive Filter + Generieren */}
            ...
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
              ...
              {activeCount > 0 && (
                <button onClick={() => setPrefs(DEFAULT_PREFS)} style={{
                  marginLeft: '0.5rem', fontSize: '0.75rem', color: 'var(--text-light)',
          ...
          <div className="container" style={{ maxWidth: '1040px' }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '620px' }}>
                <thead>
                  <tr>
                    <th style={{ width: '100px', textAlign: 'left', padding: '0.5rem 0.75rem', fontSize: '0.75rem', color: 'var(--text-light)', fontWeight: 600, borderBottom: '2px solid var(--golden)' }}>Tag</th>
                    {SLOTS.map(s => (
                  ...
                  {days.map((tag, i) => (
                    <tr key={tag} style={{ background: i % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.018)' }}>
                      <td style={{ padding: '0.6rem 0.75rem', verticalAlign: 'top' }}>
                        <span style={{ fontWeight: 700, fontSize: '0.85rem', color: 'var(--green-deep)' }}>{tag}</span>
                      </td>
                      {SLOTS.map(s => (
                        <td key={s.key} style={{ padding: '0.4rem 0.5rem', verticalAlign: 'top' }}>
            ...
            <div style={{ marginTop: '1.75rem', padding: '1rem 1.25rem', background: 'rgba(149,213,178,0.1)', border: '1.5px solid var(--mint)', borderRadius: '10px', fontSize: '0.82rem', lineHeight: 1.7, color: 'var(--text-mid)' }}>
              <strong>Noch Lücken?</strong> Mit jedem neuen Rezept das wir veröffentlichen wird der Plan vollständiger. „Rezept kommt bald" bedeutet: Kategorie passt zu deinen Filtern, das Rezept ist noch in Arbeit. 😊
            </div>
        ...
        <section className="section">
          <div className="container" style={{ maxWidth: '560px', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-mid)', marginBottom: '0' }}>
              Filter oben einstellen und auf „Plan erstellen" klicken. Die Filter werden miteinander kombiniert – z.B. Vegan + Fruktosearm + Schnell.
            </p>
          </div>
        </section>
      )}
